...
const Intelligence = () => {
    ...
    useEffect(() => {
        ...
        setDrawerOpen(true);
        // Delay slightly so the drawer/video mounts before seeking.
        const t = setTimeout(() => {
            seekTo(Number(seekSeconds) || 0);
            setSeekSeconds(null);
        }, 150);
    ...
    const generatePDFReport = () => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(0, 0, 0);
        doc.text('Forensic Executive Summary', 20, 56);

        doc.setFontSize(11);
        doc.setFont('helvetica', 'normal');
        doc.text(`Forensic Truth (Fused): ${analysisResult.metrics?.fight_probability ?? 0}% `, 20, 66);
        ...
        // Innovation: Causal Intelligence
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.text('Causal Intelligence & Interactions', 20, 92);

        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text(`Visual Quality: ${analysisResult.metrics?.is_blurry ? 'DEGRADED (Blurry)' : 'OPTIMAL'} `, 20, 100);
        const description = analysisResult.description || "Analysis Synthesized.";
    ...
    return (
        <Box sx={{ width: '100%', p: { xs: 2, md: 4, xl: 6 } }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        ...
                        {analysisResult && (
                            <Box sx={{ animation: 'fadeInUp 0.6s ease-out' }}>
                                <Paper sx={{ p: 4, borderRadius: 5, mb: 4, border: `1px solid ${alpha(theme.palette.primary.main, 0.2)}`, bgcolor: alpha(theme.palette.primary.main, 0.02) }}>
                                    ...
                                    <Typography sx={{
                                        ...
                                        lineHeight: 1.7,
                                        fontWeight: 500,
                                        fontStyle: analysisResult.description?.includes("Baseline") ? 'italic' : 'normal'
                                    }}>
                                        {analysisResult.description || "Synthesizing forensic intelligence..."}
                                    </Typography>
                                    {analysisResult.metrics?.ml_baseline !== undefined && (
                                        <Box sx={{ mt: 3, display: 'flex', gap: 2 }}>
                                            <Chip label={`ML BASELINE: ${analysisResult.metrics.ml_baseline}%`} size="small" sx={{ fontWeight: 800, bgcolor: 'rgba(255,255,255,0.05)' }} />
                                            <Chip label={`AI REFINE: ${analysisResult.metrics.fight_probability}%`} color="primary" size="small" sx={{ fontWeight: 800 }} />
                                        </Box>
                                    )}
                                </Paper>
            ...
            >
                <IntelligencePanel currentFile={file?.name} />
            </Drawer>
