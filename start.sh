...
AI_PROVIDER="${PRIMARY_VLM_PROVIDER:-ollama_cloud}"
if [ "$AI_PROVIDER" = "ollama_cloud" ]; then
    echo -e "\e[33mStarting AI Intelligence Layer (Ollama Cloud primary, local Qwen2 fallback)...\e[0m"
else
    echo -e "\e[33mStarting AI Intelligence Layer (Local Qwen2 primary)...\e[0m"
fi
...
echo -e "\e[37mAI Intelligence Layer: http://localhost:3001 (Provider Router)\e[0m"
...
echo -e ""
echo -e "\e[36mNote: Local fallback models are lazy-loaded on first fallback use\e[0m"
echo -e "\e[36mIf PRIMARY_VLM_PROVIDER=ollama_cloud, Qwen2 is not preloaded at startup\e[0m"
